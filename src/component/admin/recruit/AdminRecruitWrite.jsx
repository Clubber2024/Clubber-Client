...
export default function AdminRecruitWrite() {
  ...
  const formatDateTime = (dateObjOrStr, timeStr) => {
    ...
    const date =
      typeof dateObjOrStr === "string"
        ? dateObjOrStr
        : dateObjOrStr.toISOString().slice(0, 10);
  ...
  const uploadImages = async () => {
    //선택된 파일 배열 확정
    const filesToUpload = recruitId
      ? newAddedFiles.flat()
      : selectedFiles.flat();

    // 파일 확장자 추출
    const extensions = filesToUpload.map((file) =>
      ...
      <div className={styles.write_rectangle}>
                ...
                <div className={styles.calendar_time_div}>
                  <div className={styles.write_calendar_container}>
                    <div className={styles.input_with_icon}>
                      ...
                      />
                      <Calendar
                        className={styles.write_calendar_icon}
        ...
        <div className={styles.photo_div}>
          <div className={styles.image_preview_container}>
            <button
              className={styles.write_photo_button}
              onClick={() => inputFileRef.current.click()}
            >
              <img
                src="/admin/photo.png"
                alt="photo"
                className={styles.write_photo}
              />
            </button>
            <input
              type="file"
        ...
        <ErrorModal
          ...
          onClose={errorCloseModal}
        />
        <button
          className={styles.write_upload_button}
        ...
        {isModalOpen && isCalendarLink && (
          <RecruitFinishModal
            isOpen={isModalOpen}
            message={
              "📢모집글 수정이 완료되었습니다. 캘린더 일정도 자동으로 업데이트되었어요."
            }
