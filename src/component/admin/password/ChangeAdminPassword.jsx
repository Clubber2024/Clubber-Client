...
export default function ChangeAdminPassword() {
  ...
  const onClickPasswordChange = () => {
    ...
    if (!isPassword1) {
      setModalMessage("비밀번호는 8자 이상 20자 이하로 입력해주세요.");
      setIsModalOpen(true);
      return;
    }

    if (!isPassword2) {
      setModalMessage("영문, 숫자, 특수문자를 모두 포함해야 합니다.");
      setIsModalOpen(true);
      return;
    }

    if (!isPassword3) {
      setModalMessage("같은 문자를 3번 이상 반복할 수 없습니다.");
      setIsModalOpen(true);
      return;
    }
  ...
  const onChangeNewPassword = (e) => {
    ...
    setPasswordMessage1("비밀번호는 8자 이상 20자 이하로 설정해야 합니다.");
    setPasswordMessage2("비밀번호는 영문, 숫자, 특수문자를 포함해야 합니다.");
    setPasswordMessage3(
      "비밀번호에 동일한 문자가 3번 이상 연속될 수 없습니다."
    );
      ...
      <div className={styles.password_total_div}>
        ...
        <div className={styles.password_content_div}>
          ...
          <div className={styles.content_pw_div}>
            ...
            <img
              src={
                isShowCurrentPwChecked
                  ? "/admin/sign-up/eye.png"
                  : "/admin/sign-up/eye-off.png"
              }
        ...
        <div className={styles.password_content_div}>
          ...
          <div className={styles.password_msg_container}>
            <p
              className={isPassword1 ? styles.message_confirm : styles.message}
            >
              {" "}
              {passwordMessage1}{" "}
            </p>
            <p
              className={isPassword2 ? styles.message_confirm : styles.message}
            >
              {" "}
              {passwordMessage2}{" "}
            </p>
            <p
              className={isPassword3 ? styles.message_confirm : styles.message}
            >
              {" "}
              {passwordMessage3}{" "}
            </p>
        ...
        <div className={styles.password_content_div}>
          ...
          <p
            className={
              isPasswordConfirm ? styles.message_confirm : styles.message
            }
          >
            {" "}
        ...
        <button
          ...
          className={styles.password_confirm_button}
        >
          확인
        </button>
      </div>
