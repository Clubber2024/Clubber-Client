    ...
    try {
      ...
      if (response.data.success) {
        ...
        const favorite = data.find(
          (item) => item.favoriteClub["clubId"] === intClubId
        );
    ...
    try {
      if (isFavorite) {
        const res = await customAxios.delete(
          `/v1/clubs/${intClubId}/favorites/${favoriteId}`,
          {
            headers: {
      ...
      if (!isFavorite) {
        const addres = await customAxios.post(
          `/v1/clubs/${intClubId}/favorites`,
          favoriteData,
          {
            headers: {
              "Content-Type": "application/json",
  ...
  return (
    <div className="detail_container">
        ...
        <div className="detail_header">
          <div className="detail_header_name">
            ...
            <img
              className="star_icon"
              src={
                isFavorite ? "/bookmark/starYellow.png" : "/bookmark/star.png"
              }
              alt="star"
      ...
      <div className="detail_tab">
        <button
          className={whichTab === "Introduction" ? "active" : ""}
          onClick={onClickIntroTab}
        >
          소개
        </button>
        <button
          className={whichTab === "Review" ? "active" : ""}
      ...
      {whichTab === "Review" && (
        <ReviewPage clubId={clubId} clubName={detailData.clubName} />
      )}
    </div>
  );
}
